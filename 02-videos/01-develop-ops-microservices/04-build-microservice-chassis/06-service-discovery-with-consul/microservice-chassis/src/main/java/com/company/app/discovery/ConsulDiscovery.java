...
public class ConsulDiscovery implements ServiceDiscovery {
    private ConcurrentHashMap<String, List<HostAndPort>> services = new ConcurrentHashMap<>();
    ...
    public HostAndPort discover(String service) {
        ...
        return registrations != null ? registrations.get(random.nextInt(registrations.size())) : doDiscover(service);
    }
    ...
    private HostAndPort doDiscover(String service){
        final ConsulResponse<List<ServiceHealth>> response = healthClient.getHealthyServiceInstances(service, QueryOptions.BLANK);
        AtomicReference<Long> index = new AtomicReference<>(response.getIndex());
        update(service, response);
        healthClient.getHealthyServiceInstances(service, QueryOptions.BLANK, new ConsulResponseCallback<List<ServiceHealth>>() {
            @Override
            public void onComplete(ConsulResponse<List<ServiceHealth>> consulResponse) {
                update(service, consulResponse);
                index.set(consulResponse.getIndex());
                healthClient.getHealthyServiceInstances(service, builder().blockMinutes(1, index.get()).build(), this);
            }
    ...
    /**
     * Updates the internal map of service registrations with those specified in Consul response.
     *
